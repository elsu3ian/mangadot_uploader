...
def parse_filename_details(filename, upload_type="chapter"):
    ...
    # Split by ' - ' up to 2 times to isolate Ch, Ep, and the actual Title
    parts = name_without_ext.split(' - ', 2)
    
    if len(parts) == 3:
        # Matches: Ch.093 - Ep. 92 - Gladiator -> Title: "Gladiator"
        title = parts[2].strip()
        number_str = parts[0]
    elif len(parts) == 2:
        # Matches: Ch.093 - Gladiator -> Title: "Gladiator"
        title = parts[1].strip()
        number_str = parts[0]
    else:
        title = None
        number_str = name_without_ext
...
def upload_file_tus_worker(session, renderer, file_info, manga_id, group_ids, upload_type, batch_id, language, scanlator_name):
    ...
    tus_metadata = {
        ...
        "chapter_title": file_info["title"],  # THIS LINE ADDS THE CHAPTER TITLE TO THE METADATA, WHICH MANGADOT CAN USE FOR BETTER DISPLAY
        "language": language,
        "group_ids": group_ids,
